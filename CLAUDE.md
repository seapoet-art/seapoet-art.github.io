...
- `content/poetryprints.html` — the single poetry-prints gallery page (linked from `index.html`'s "View Prints" button). Every poem lives here as a `<figure class="poem" data-tags="...">` — `data-tags` holds one or more space-separated category slugs (`political`, `sobriety-mental-health`, `ocean`, `miscellaneous`; a poem spanning multiple categories just gets multiple slugs, e.g. `data-tags="ocean sobriety-mental-health"`). A client-side tag filter (`js/poetry-filter.js`) lets viewers narrow the view to one category at a time via pill buttons; without JS every poem stays visible (progressive enhancement, not a hard requirement).
